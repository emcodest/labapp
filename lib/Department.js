...
this.ListTestDialog = async (req, res) => {
    ...
    for (let r of list_cat) {
        dresult.push([r.test_code, r.test_name,  "<button params = '"+r.test_id+"' func = 'AddtestToDialog'  class = \"my-btn click\"><i class = \"fa fa-plus-circle\"></i></button>", r.id])
    }
...
this.ListProfileDialog= async (req, res) => {
    ...
    var list_dept = await handler.bindRawQuery("select * from profile order by name asc limit 1000", {})
    for (let r of list_dept) {
        dresult.push([r.code, r.name, "<button params = '"+r.id+"' func = 'AddToAcceptTest'  class = \"my-btn click\"><i class = \"fa fa-plus-circle\"></i></button>", r.id])
    }
