...
_this.PatientRecords = async (req, res) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        handler.bindRawQuery("select * from patient_master order by first_name asc limit 10000", {}).then(rows => {
            ...
            for(let r of rows){
                dresult.push([r.code, r.first_name+" "+r.last_name, r.sex, r.phone_number, r.dob, r.email, r.religion, r.nationality, r.state_origin,  "<button = \"alert('baddest')\" class = \"my-btn click\" params = '"+r.id+"' func = 'AddToAceeptTest'><i class = \"fa fa-plus\"></i></button>", "<button params = '"+r.id+", patient_master'  func = 'DeleteMaster' class = \"my-btn click\"><i class = \"fa fa-times \"></i></button>", "<button params = '"+r.id+", edit_patient_master'  func = 'EditMaster' class = \"my-btn click\"><i class = \"fa fa-edit \"></i></button>" ])
            }
            // console.log({
            //     data: dresult
