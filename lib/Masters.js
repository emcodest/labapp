...
this.ListRefRanges = async (req, res) => {
    ...
    var list_master = await handler.bindRawQuery("select id, test_id, unit_1, unit_2, lower_limit_1 as lb_1, lower_limit_2 as lb_2, upper_limit_1 as up_1, upper_limit_2 as up_2, sex, date_type, from_age, to_age  from ref_ranges where test_id = $test_id", {test_id})
...
this.AcceptTest = async (req, res) => {
    ...
    if(is_test){
        ...
        // extract tests
        var key_len = 10
        var list_test = handler.NormalizeArray(JSON.parse(data.test_raw), "test_info", ["test_id", "test_name"], key_len) 
       
        for(var i in list_test){
            await handler.genInsert({accepted_test_id: accepted_test_id, test_id: list_test[i].test_id, status_approve: "no", status_partial: "no", status_full: "no", status: "unapproved"}, "perform_tests")
        }
...
this.ListAcceptedTests = async (req, res) => {
    //var {master_name} = req.body
    var dresult = []
    var list_master = await handler.bindRawQuery("select *, strftime('%d-%m-%Y %H:%S', created_at) as date, (select sex from patient_master where code = x.patient_id) as sex, (select age from patient_master where code = x.patient_id) as age  from accepted_tests x order by id desc limit 5000", {})
    for (let r of list_master) {
        dresult.push([r.date, r.lab_no, r.patient_id, r.sex, r.age, "<button params = '"+r.id+"'  func = 'EditAcceptedTest' class = \"my-btn click\"><i class = \"fa fa-edit \"></i></button>", "<button params = '"+r.id+", accepted_tests'  func = 'DeleteMaster' class = \"my-btn click\"><i class = \"fa fa-times \"></i></button>"])
    }
...
this.ListAcceptedTestsUnpartial = async (req, res) => {
    ...
    for (var r of list_master) {
        ...
        for(var ii in mtests){
            save_info += "<li>"+mtests[ii].test_name+" <b>("+mtests[ii].code+")</b>"+"</li>"
        }
        save_info += "</ol>"
        r.code = save_info
...
this.SortDepartment = async (req, res) => {
   ...
   // console.log("######### "+ dept_id)
    ...
    for (var r of list_master) {
        ...
        dresult.push([r.date, r.lab_no, r.sex, '<a class = "pop" href="#"  data-title="Age" data-content="'+r.age+'" data-placement="right"><i class = "fa fa-history"> view</a>', '<a class = "pop" href="#"  data-title="Referred By" data-content="'+r.ref+'" data-placement="right"><i class = "fa fa-history"> view</a>', '<a class = "pop" href="#"  data-title="Tests" data-content="'+r.code+'" data-placement="right"><i class = "fa fa-history"> view</a>', "<button params = '"+r.id+"'  func = 'PerformTest' class = \"my-btn click\"><i class = \"fa fa-history \">Perform</i></button>", "Performed <input style = 'width: 20px; height: 20px; display: block;' type = 'checkbox' />"])
    }
